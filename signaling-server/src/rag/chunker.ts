...
function packUnits(units: string[], targetChars: number): string[] {
  const out: string[] = []
  let cur: string[] = []
  let curLen = 0
  ...
  for (const unit of units) {
    if (unit.length > targetChars) {
      ...
      if (sCur) out.push(sCur)
      continue
    }
    if (curLen + unit.length + 1 > targetChars) flush()
    cur.push(unit)
...
function withOverlap(windows: string[], overlapChars: number): string[] {
  if (overlapChars <= 0 || windows.length < 2) return windows
  const result: string[] = [windows[0]]
